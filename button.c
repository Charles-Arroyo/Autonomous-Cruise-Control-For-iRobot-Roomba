...
void init_button_interrupts() {
    ...
    GPIO_PORTE_ICR_R = (GPIO_PORTE_ICR_R & 0b000001111) && (~GPIO_PORTE_ICR_R & 0b00001111);
...
uint8_t button_getButton() {
	...
	// INSERT CODE HERE!
    // Check each button from left to right
    uint8_t buttonState = GPIO_PORTE_DATA_R & 0x0F;
        if ((buttonState & 0x08) == 0) {
           return 4; // Button 1 is pressed
       } else if ((buttonState & 0x04) == 0) {
           return 3; // Button 2 is pressed
       } else if ((buttonState & 0x02) == 0) {
           return 2; // Button 3 is pressed
       } else if ((buttonState & 0x01) == 0) {
           return 1; // Button 4 is pressed
       } else {
